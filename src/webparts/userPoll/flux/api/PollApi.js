...
const API = {
    ...
    checkForAnswers(endPoint, pollId){
        ...
        getCurrentUser.done(function(data){
            ...
            var username = data.d.Title;
            var promise = $.ajax({
                url: "https://asuep.sharepoint.com/sites/DeviLink/_api/web/lists/GetByTitle('User Poll Results')/items?$filter=PollUserPollResultId eq " + pollId + "&UserUserPollResultsId eq " + userID + "",
                method: "GET",
                contentType : "application/json",
                dataType : 'json',
                beforeSend : function(xhr){
